...
pub fn mixColor32bgra(a: u32, b: u32, t: u16) u32_bgra {
    const a_b: u8 = @intCast(a & 0xff);
    const a_g: u8 = @intCast((a >> 8) & 0xff);
    const a_r: u8 = @intCast((a >> 16) & 0xff);
    const a_a: u8 = @intCast((a >> 24) & 0xff);
    const b_b: u8 = @intCast(b & 0xff);
    const b_g: u8 = @intCast((b >> 8) & 0xff);
    const b_r: u8 = @intCast((b >> 16) & 0xff);
    const b_a: u8 = @intCast((b >> 24) & 0xff);
    const c_b = math.lerpFixed1024(a_b, b_b, t);
    const c_g = math.lerpFixed1024(a_g, b_g, t);
    const c_r = math.lerpFixed1024(a_r, b_r, t);
    const c_a = math.lerpFixed1024(a_a, b_a, t);
...
pub fn mixByU8(a: u32, b: u32, t: u8) u32_bgra {
    ...
    const b_r: u8 = @intCast((b >> 16) & 0xff);
    const b_a: u8 = @intCast((b >> 24) & 0xff);
    const c_b = math.lerp8bit(a_b, b_b, t);
    const c_g = math.lerp8bit(a_g, b_g, t);
    const c_r = math.lerp8bit(a_r, b_r, t);
    const c_a = math.lerp8bit(a_a, b_a, t);
    return makeColor32bgra(c_r, c_g, c_b, c_a);
}
